...
public class CocheValidation implements ValidationStrategy {
    @Override
    public ValidationResult validate(VehicleData data) {
        ...
        if (data.getModelo() == null || data.getModelo().trim().isEmpty()) {
            return new ValidationResult(false, "Para Carro el campo Modelo es obligatorio.");
        }
        if (data.getAnio() == null || data.getAnio().trim().isEmpty()) {
            return new ValidationResult(false, "Para Carro el campo año es obligatorio.");
        }
        if (data.getPlaca() == null || data.getPlaca().trim().isEmpty()) {
            return new ValidationResult(false, "Para Carro el campo Placa es obligatorio.");
        }
        if (data.getColor() == null || data.getColor().trim().isEmpty()) {
            return new ValidationResult(false, "Para Carro el campo Color es obligatorio.");
        }
